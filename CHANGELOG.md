...
+ [修复] 生图与生视频接口返回错误对象时，画布节点展示上游具体错误内容。

+ [优化] OpenAI 兼容生图与改图请求改为返回图片 URL，减少 Base64 响应体积与传输耗时。

+ [修复] 画布异步视频任务会保存并显示上游任务 ID，页面恢复后继续轮询，并支持手动拉取最新状态。

+ [调整] 视频生成时长上限提升至 30 秒，生图与生视频提示词输入框增加 4000 字符限制和实时字数统计。

+ [修复] MiniMax H3 视频模型按接口要求限定 1440P、画面比例与 5–15 秒时长，并以 `resolution + ratio` 方式提交参数。

+ [修复] 视频轮询支持解析无状态码的数据包装及 `fail_reason`，任务失败时展示上游返回的具体原因。

+ [新增] 画布生图与生视频节点会分别记住最近一次生成配置，并在后续新增节点时自动沿用。

+ [修复] 修复火山方舟渠道的视频模型可能错误使用 OpenAI 请求格式的问题。

+ [修复] 修复移动模式下节点点击被画布平移拦截，以及鼠标中键拖动画布失效的问题。
+ [调整] 视频生成轮询不再受固定时长限制，持续等待上游任务完成或明确失败。
+ [修复] 保留视频生成失败的具体错误信息，并支持按住鼠标滚轮拖动画布。
+ [调整] 画布无选中节点时自动使用移动模式，选中节点后自动切换为选择模式并支持手动切换。
...
+ [新增] 画布空白处右键弹出创建节点菜单，可快速新建文本、图片、视频、音频等节点。
+ [调整] 画布批量生图改为独立节点模式：每张图各自创建独立节点并分别连线，不再合并为单个合集节点。
