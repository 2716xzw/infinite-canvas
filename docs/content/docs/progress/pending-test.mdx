...
- MiniMax H3 video parameters: selecting the model should fix resolution to 1440P, offer only the six supported ratios and 5–15 second durations, then submit `resolution`, `ratio`, and `duration` without also submitting the mutually exclusive `size` parameter.
- Video polling error details: task responses wrapped as `{ data: task, fail_reason }` without a top-level `code` should be unwrapped correctly, and failures should display the provider's `fail_reason` or nested `data.error.message` instead of a generic generation error.
- Canvas generation configuration memory: after changing image quality, size, count, or background, newly added Image and Config nodes should reuse those image settings; after changing video resolution, ratio/size, duration, audio, or watermark, newly added Video nodes and Config nodes switched to Video should reuse those video settings independently across page refreshes.
...
- Video generation polling: long-running upstream tasks should continue polling beyond the previous limit and finish when the task succeeds, reports failure, expires upstream, or is cancelled by the user.
...
- Volcengine Ark video routing: a video model selected from an Ark channel should create tasks through `/contents/generations/tasks` with the Ark JSON payload, including when another channel contains a model with the same name.
